    ...
    if ( nres.workface != undefined ) {
      var oldWorkFace = Workface.findOne({_id: nres.workface}).exec(function (error, oldworkface) {
        ...
        var newWorkFace = Workface.findOne({_id: req.body.workface}).exec(function (error, newworkface) {
            ...
            var nArticle = Article.findOne({_id: nres.article}).exec(function (error2, article) {
              if(article != undefined && article.price != undefined) {
                  var aprice = article.price - parseInt(oldPrice) + parseInt(newprice);
                  Article.where({_id: article._id}).update({ $set : { "price" : aprice }}).exec(function(er, resp){
                      console.log(er, resp);
                  });
              }
            });
        ...
        if(req.body.type == "closing") {
            Note.findOne({
                ...
                if (err) {
                 ...
                 mongoose.connection.db.collection('articles').findOneAndUpdate(
                     { "_id" : user.article },
                     { $set: { "DateClosedFocusInformation" : "date" } }
                 );
                ...
                MongoClient.connect('mongodb://127.0.0.1:27017/test', function(err, db) {
                    ...
                    db.collection('articles').findAndModify(
                        {_id: "5845fc4d7caa400f00604fc7"}, // query
                        {$set:{"DateClosedFocusInformation.value":"2017-5-11"}},
                        {update:true},
                        function(err, object) {
                            if (err){
...
exports.delete = function(req, fres) {
  ...
  var xnote = Note.findOne({ _id: req.params.noteId }).exec(function (err, res) {
    ...
    if ( res.workface != undefined && res.article != undefined && res.article != '' ) {
      var nWorkFace = Workface.findOne({_id: res.workface}).exec(function (error, workface) {
        var nArticle = Article.findOne({_id: res.article}).exec(function (error2, article) {
          var aprice = article.price - parseInt(workface.price)
          Article.where({_id: article._id}).update({ $set : { "price" : aprice }}).exec(function(er, resp){
            console.log(er, resp);
  ...
  query.sort('-created').exec(function(err, notes) {
    if (err) {
      ...
      return res.status(400).send({
		...
		/* var cck = [];
		for(var nk in notes){ 
			if(typeof notes[nk].article != "undefined"){
				Article.findOne({_id: notes[nk].article}).exec(function (error2, articleData) {			
					if(typeof articleData.StatusFocusInformation.value != "undefined"){
						console.log(articleData.StatusFocusInformation.value)
						cck.push(articleData.StatusFocusInformation.value);
					}
				});				
			}
		}
