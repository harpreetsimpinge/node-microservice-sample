...
exports.getStatus = function (req, res) {
  var Article = mongoose.model('Article');
    ...
    Article.findOne({_id : req.body.articleId }, function(err, doc){
        if (err){
...
exports.updateStatus = function (req, res) {
  var Article = mongoose.model('Article');
    Article.update({_id : req.body.articleId }, req.body, {upsert:false}, function(err, doc){
        if (err){
            res.send(500, { error: err });
            return;
        } else {
...
exports.list = function (req, res) {
  ...
  //
  //         for(var linkedArticles in noteResponse) {
  //             Article.find({'_id': noteResponse[linkedArticles].article}).lean()
  //                 .limit(1000)
  //                 .exec(function (err, allarticles) {
  //                     for(var i in allarticles) {
  //                         newArticleArray.push(allarticles[i]);
  //                     }
  //                 });
  //         }
  //     });
  ...
  if(req.user.roles[0] === 'user') {
              Article.find({'user': req.user._id}).lean()
                  .limit(1000)
                  .exec(function (err, allarticles) {
                      for(var i in allarticles) {
                          newArticleArray.push(allarticles[i]);
